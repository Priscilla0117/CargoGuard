...
The [live CargoGuard application](https://cargoguard-averis.onrender.com/) currently runs commit `22d1ccf`, engine **3.4.1**, deployed on **24 September 2026** with [101 bounded hosted assertions passing](docs/CLOUD_RELEASE_341.md). A subsequently reproduced Outlook-to-work-queue navigation crash is corrected on this final-round branch, alongside a shared navigation layout; **deployment and final hosted acceptance of that correction are pending**. See the [navigation release record](docs/NAVIGATION_RELEASE_20260924.md). The public service retains isolated **demo workspaces with organiser samples**, not employee-team or corporate SSO access. Main remains unchanged and automatic deployment is off.
...
Email content is untrusted evidence, never an instruction to the application. Recognized lines addressed to software ("ignore previous instructions", "mark as verified") are removed before routing and shown to the employee as a warning; deterministic comparison remains independent of those instructions. Spam attachments stay unopened at intake. Placeholder values such as `TBA / TBC` or `TO BE ADVISED (TBA)` go to review even when both documents agree. Each mismatch has a plain-language explanation of a possible clerical pattern (for example swapped digits, tonnes vs kg, or a port code kept while the name changed) or a different value requiring confirmation. These suggestions are not confirmed causes. The original SI comparison remains visible when an employee approves a later instruction. Missing, damaged, ambiguous or scanned documents cannot become automatic matches. OCR suggestions include source crops and recognition scores; all seven fields require human confirmation.
...
On engine 3.4.1, three fresh first-run seeds (**219971, 329983, 439997**) produced **1,560/1,560 exact outputs**, catching all **154 defects** and **60 required reviews**, with **0 false clearances among 162 verified cases**. All 29 files in the frozen engine/evaluation dependency set remained unchanged during these runs. The evaluator now uses the application's actual intake and processing path. These datasets share the organiser generator: results establish within-generator consistency, not real-world accuracy, absence of bugs or a judging score. Human-assisted results are excluded from the untouched automatic benchmark. See the [current audit](docs/JUDGE_FEEDBACK_AUDIT.md), [pilot measurement workflow](docs/PILOT_EVALUATION.md) and [model card](docs/MODEL_CARD.md). Historical 3.3.1 measurements remain in [their dated report](docs/FINAL_ENHANCEMENTS_VALIDATION.md).
