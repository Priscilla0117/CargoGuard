...
Optional Evidence Recovery Copilot handles unfamiliar readable layouts. With explicit organiser/synthetic-data consent, Render sends bounded extracted lines to the fixed OpenAI Responses endpoint using an owner-supplied server-side key. The browser never receives the key. The LLM selects verbatim value fragments; the server validates citations and units. All seven fields and the role require explicit reviewer confirmation before a hash/revision-bound proposal can become a reviewed source revision. The separate case assistant offers bounded, source-linked chat after preview and consent. Persistent Turso reservations enforce shared usage limits, including failures. Provider errors preserve saved decisions. Neither feature can send messages or approve shipments; see [AI_UPGRADE.md](AI_UPGRADE.md) and [CASE_ASSISTANT.md](CASE_ASSISTANT.md).
...
| POST /api/cases | Process up to 10 IDs, correct a field, confirm category/scan transcription, or select a document pair | Bounded schema/body, origin check, optimistic version; explicit source-bound review |
...
| POST /api/upload | Import email attachments, replace the source set, or final-round BL-only replacement with multipart mode=replace_bl | Up to ten files, 5 MiB/file, 20 MiB combined, four allowed extensions, random immutable source keys; BL-only replacement retains the selected server-side SI |
...
| GET/POST /api/recovery | Show availability, request source-quoted AI suggestions and confirm a proposal | Consent, bounded source text, shared persistent allowance, source hashes/current revision and seven confirmations |
| GET/POST /api/assistant | Show allowance, preview outgoing context and ask a case question | Explicit consent, current case revision/context hash, bounded single-case context, shared allowance; answers cannot mutate cases |
| GET/POST /api/follow-ups (final-round branch) | Read/save operational follow-up; GET ?export=1 downloads a text handoff | Workspace scope, independent follow-up concurrency, decision revision/engine binding, completion gate; no comparison mutation or sending |
...
- XLSX: read workbook relationships and literal shared/inline strings and values. Formulas and error cells require a recalculated, inspected values-only export; stale cached formula results are not independently verified. Cell references accompany extracted values.
...
Implemented: HttpOnly/SameSite cookie, Secure in production, no-store responses, parameterized SQL, stale-write detection, ZIP expansion limits, file-type limits, escaped React text and SHA-256 file fingerprints. No email is sent automatically. Ordinary routing, parsing, comparison and browser OCR do not call an external AI service; optional recovery and case chat send bounded content only after explicit consent. A source quotation proves the text exists, not that the model assigned it the right meaning.
...
## Final-round follow-up extension

The final-round branch adds persistent operational follow-up alongside comparison evidence: self-declared owner, shipment reference, due date and open/waiting/completed state. Waiting/completed follow-ups appear reopened when their recorded revision differs or the case engine is outdated; already-open follow-ups remain open. Completion requires seven valid matching fields from the current engine. Tracking updates do not change the strict document verdict, source evidence or automatic evaluation output. Follow-up changes and handoff exports preserve the decision reference for review. BL-only replacement retains the selected server-side SI rather than requiring a reviewer to upload its replacement.

This is workspace-scoped follow-up, not a shared employee directory, authenticated assignment system, automatic carrier dispatch or cargo-release approval. Local acceptance passed on `codex/final-round-employee-workflow`; hosted deployment remains pending and no GitHub push was made. [FINAL_ROUND_VALIDATION.md](FINAL_ROUND_VALIDATION.md) records the local gate, HTTP checks and browser correction/recovery journey separately from dated 3.2.1 cloud evidence. See [FINAL_ROUND_STRATEGY.md](FINAL_ROUND_STRATEGY.md) for acceptance cases and adoption, and [the synthetic rehearsal](../examples/final-round/README.md).
