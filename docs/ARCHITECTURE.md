...
The optional Evidence Recovery Copilot assists unfamiliar readable layouts. With explicit organiser/synthetic-data consent, Render sends bounded extracted lines to the fixed OpenAI Responses endpoint using an owner-supplied server-side key. The browser never receives the key. The LLM selects verbatim value fragments; the server validates citations and units. All seven fields and the role require explicit reviewer confirmation before a hash/revision-bound proposal can become a reviewed source revision. Persistent Turso reservations enforce shared usage bounds, including failed attempts. Provider errors preserve the saved decision. This field-recovery workflow is separate from the case chatbot; see [MODEL_CARD.md](MODEL_CARD.md).
