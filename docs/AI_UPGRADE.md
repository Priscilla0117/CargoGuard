...
Approved usage is limited to the owner's existing API balance; no purchase, subscription upgrade or automatic top-up is included. Following the owner's September 21 request, daily capacity is now 50 attempts globally per UTC day, 10 per workspace/day and 500,000 conservatively reserved token units/day. The original lifetime ceilings remain 100 attempts and 1,000,000 token units. Two concurrent calls, 3,072 maximum recovery output tokens and a 25-second provider timeout remain. These are application bounds, not provider quota or account-wide billing promises. Other limits can stop calls sooner. Failed calls consume quota; restarting Render or resetting cookies does not refund it. Public traffic can exhaust the shared allowance. Never remove attempt records to bypass the cap. See WORKSPACE_REFRESH.md for the quota diagnosis and request-specific preview.
