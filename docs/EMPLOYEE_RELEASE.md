...
- **Amendment resolution:** follow an approved email instruction into the actual revised SI, retain reviewer incorporation evidence, and keep every remaining BL difference visible. Editing a comparison field cannot stand in for a revised document. Request drafts, decision briefs and source-version guards support handover. See the [rehearsal files](../examples/amendment-resolution/README.md) and [value proposition](CREATIVE_VALUE_PROPOSITION.md).
...
|---|---|---|
| Latest full quality gate | **531/531 automated tests**, TypeScript, ESLint, input integrity, organiser evaluation, independent scoring, local OCR assets and production build: **all eight steps passed** | `work/validation/quality-gate.json`, `unit-tests.log` and the adjacent step logs |
| Previous release API acceptance | **178 passing checks** across five suites, covering source/revision integrity, workspace boundaries, review, shipments, deadlines, templates, batch actions and untrusted email content | `work/api-test-report.json`; `work/validation/local-hardening-api.json`, `v32/review-workspace-api.json`, `final-round/follow-up-api.json`, `final-operations/local-operations-api.json` |
| Production team acceptance | **49 checks across 44 HTTP requests**, including full restart with bootstrap secret removed; sessions, sources, revisions, audit and shipment data persisted | `work/validation/team-runtime.json` |
| Amendment HTTP acceptance | **18 assertions across 39 requests**: original-SI reuse and manual SI-edit bypass rejected; wrong revised SI rejected; unrelated BL error blocks completion; corrected evidence can complete; stale writes and workspace access checked | `work/validation/amendment-api.json` |
| Authored operations challenge | **20/20** strict comparison and **20/20** independent-check expectations passed; five mismatches, four mandatory reviews and five independent-attention scenarios | [Dataset provenance and reproduction](OPERATIONS_CHALLENGE.md); [aggregate report](../public/validation.json) |
| Amendment browser acceptance | Reviewer incorporation, wrong-port completion rejection, changed comparison requiring renewed review, corrected completion and actual downloaded decision brief verified; desktop 1366×900 and mobile 390×844 inspected without page overflow | Synthetic rehearsal in local production build; [rehearsal instructions](../examples/amendment-resolution/README.md) |
