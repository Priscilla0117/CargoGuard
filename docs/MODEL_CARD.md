...
126 regression tests cover independent field mutations; whole-expression and 400 generated compound-count checks; placeholders; duplicate labels; misleading subjects and quoted threads; correction dependencies; uncertain routing; bounded parsing; source identity; scan confirmation; policies; immutable storage transactions; reverse-proxy origin safety; and malformed requests. Run `npm run quality` for retained logs.
