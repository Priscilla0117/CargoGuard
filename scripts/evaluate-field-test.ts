...
const emlDir = args.includes("--eml-dir")
  ? args[args.indexOf("--eml-dir") + 1]
  : "";
...
for (const scenario of FIELD_TEST) {
  ...
  if (scenario.expect.outcome) {
    ...
    if (scenario.expect.outcome === "OK")
      score(
        "no_false_alarm_on_matching_documents",
        scenario.id,
        result.status === "OK",
        "OK",
        result.status,
...
for (let i = 0; i < ids.length; i++)
  for (let j = i + 1; j < ids.length; j++) {
    const a = FIELD_TEST[i],
      b = FIELD_TEST[j];
    const expected = !!a.thread && a.thread === b.thread;
    const actual =
      !!threads.get(a.id) && threads.get(a.id)?.key === threads.get(b.id)?.key;
    if (expected && actual) tp++;
    else if (actual) {
      fp++;
      failures.push({
        id: `${a.id}+${b.id}`,
        check: "conversation_grouping",
        expected: "separate",
        actual: "grouped",
      });
    } else if (expected) {
...
const plans = summaries.map((row) => ({
  row,
  plan: planFor(row, undefined, NOW),
}));
const ranked = plans
...
const top = ranked
  .slice(0, urgent.length + 2)
  .map(({ row }) => row.email.email_id);
for (const id of urgent)
  score(
    "urgent_email_in_top_of_plan",
    id,
    top.includes(id),
    `rank ≤ ${urgent.length + 2}`,
    `rank ${ranked.findIndex(({ row }) => row.email.email_id === id) + 1}`,
  );
for (const { row, plan } of plans) {
  ...
  const want = new Date(BASE.getTime() + scenario.expect.deadline * 86400000)
    .toISOString()
    .slice(0, 10);
  const found = row.email.insight?.dates.some(
    (d) => d.date === want && d.kind !== "Mentioned",
  );
  score(
    "deadline_found_in_email",
    scenario.id,
    !!found,
    want,
    row.email.insight?.dates.map((d) => `${d.kind} ${d.date}`).join("; ") ||
      "none",
  );
  void plan;
}
